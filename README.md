...
## Сбор метрик в приложении

Приложение автоматически собирает метрики для Prometheus через OpenTelemetry.  

**Что мониторится:**
- Все основные middleware:
  - `auth` — успешные и неуспешные авторизации
  - `ratelimit` — количество разрешённых и заблокированных запросов
  - `slidingwindow` — количество запросов в скользящем окне
  - `session` — активные и истёкшие сессии
  - `stateupdate` — успешные и неудачные обновления состояния
  - `queue` — обработка задач из очереди
  - `pagecounter` — количество просмотров страниц
  - `distributedlock` — количество успешных и заблокированных операций
  - `recovery` — количество перехваченных паник  

Метрики доступны на эндпоинте `/metrics` и могут опрашиваться Prometheus.  
Таким образом, вы получаете полную картину работы приложения и состояния основных middleware.
